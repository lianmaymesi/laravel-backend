...
<div class="grid gap-y-1.5" {{ $attributes->whereStartsWith('x-ref') }}>
    ...
    @endif
    <div class="w-full max-w-full" x-data="{ ...choiceSelect(@entangle($attributes->wire('model')), @js($options), @js($value)) }" wire:ignore x-cloak>
        <div class="min-w-0 flex-1">
            <select x-ref="selectdoc" multiple></select>
        </div>
    </div>
    @if ($helpText)

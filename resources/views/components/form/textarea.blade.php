    ...
    <div
        ...
        <div class="flex-1 min-w-0">
            <textarea {{ $attributes }} aria-invalid="{{ $error ? 'true' : 'false' }}"
                class="block w-full border-none bg-transparent py-1.5 pe-3 ps-3 text-sm leading-6 text-slate-950 outline-hidden transition duration-75 placeholder:text-slate-500 focus:ring-0 disabled:text-slate-500 disabled:placeholder:text-slate-400">{{ $slot }}</textarea>
        </div>
    </div>
    @if ($helpText)

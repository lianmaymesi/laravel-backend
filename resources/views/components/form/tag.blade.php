...
<div x-data="{
        ...
        <div @click.away="clearSearch()" class="w-full" @keydown.escape="clearSearch()">
            ...
            <div :class="[open ? 'block' : 'hidden']">
                <div class="absolute left-0 z-9999 mt-2 w-full">
                    <div
                        class="py-1 text-sm bg-white border border-gray-300 rounded-sm shadow-lg dark:border-slate-700 dark:bg-slate-900">
    ...
    <div class="mt-0.5 flex flex-wrap gap-2 overflow-hidden">
        <template x-for="(tag, index) in tags">
            <div class="inline-flex items-center gap-1 text-xs font-medium text-white bg-indigo-600 rounded-sm">
                <span
                ...
                <button @click.prevent="removeTag(index)"
                    class="inline-block w-6 h-6 text-indigo-100 align-middle hover:text-white focus:outline-hidden">

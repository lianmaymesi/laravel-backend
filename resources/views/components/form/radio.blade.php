...
<div @class(['grid', 'gap-y-1.5' => $helpText || $error])>
    <label class="inline-flex items-center">
        <input type="radio" {{ $attributes }} aria-invalid="{{ $error ? 'true' : 'false' }}"
            class="text-indigo-600 border-gray-300 rounded-full shadow-xs focus:border-indigo-300 focus:ring-0 focus:ring-indigo-200 focus:ring-opacity-50 focus:ring-offset-0" />
        @if (!$labelOff)
